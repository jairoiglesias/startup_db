...
SECURITY_STATUS SEC_ENTRY _sspi_FreeContextBuffer(void* pvBuffer){

  SECURITY_STATUS   status;

  printf("Processing FreeContextBuffer ...\n");

  freeContextBuffer_fn pfn_freeContextBuffer; 

  // Recupera o ponteiro para a função FreeContextBuffer da DLL
  pfn_freeContextBuffer = (void *)GetProcAddress(_sspi_security_dll, "FreeContextBuffer");

  printf("Pointer Function FreeContextBuffer: %p\n", pfn_freeContextBuffer);

  status = (*pfn_freeContextBuffer)(pvBuffer);
...
SECURITY_STATUS SEC_ENTRY _sspi_FreeCredentialsHandle(
  ...
  printf("Processing FreeCredentialsHandle ...\n");
  ...
  // printf("Processing GetProcAddress ...\n");
  ...
  #else
      printf("NOT UNICODE!\n");
      pfn_freeCredentialsHandle = (_sspi_FreeCredentialsHandle_fn)GetProcAddress(_sspi_secur32_dll, "FreeCredentialsHandle");
  #endif

  printf("Pointer Function FreeCredentialsHandle: %p\n", pfn_freeCredentialsHandle);
...
SECURITY_STATUS SEC_ENTRY _sspi_DeleteSecurityContext(PCtxtHandle phContext) {
  ...
  printf("Pointer Function DeleteSecurityContext: %p\n", pfn_deleteSecurityContext);

  // Check if the we managed to map function pointer
  if(!pfn_deleteSecurityContext) {
    printf("GetProcAddress failed _sspi_DeleteSecurityContext.\n");
    return -2;
  }

  printf("Pointer Context: %p\n", phContext);

  status = (*pfn_deleteSecurityContext)(phContext);

  printf("_sspi_DeleteSecurityContext Returned !\n");
